...
function openModal(id) {
	...
	$('#modal').modal('show');		
	$('#modal .modal-content').load("/ContaWeb_Prova_Fabio/ddt.do?idDDT=" + id, function(){
...
function newDdtWithLoad(){
	$('#modal .modal-content').load("/ContaWeb_Prova_Fabio/ddt.do?idDDT=0", function(){
		newDdt();
	});
}
...
function getClienti(){
	...
	$('#clientilist').load("/ContaWeb_Prova_Fabio/clientiFindList.do");
}
...
function setCliente(){
	...
	$.ajax({
		  ...
		  dataType: "json",
		  url: "/ContaWeb_Prova_Fabio/cliente.do?id=" + id
		})
		.done(function( data ) {
...
function getPuntoConsegna(type, id)
{
	...
	$.ajax({
		  ...
		  contentType: "application/json",
		  dataType: "json",
		  url: "/ContaWeb_Prova_Fabio/clienteDestinazione.do?id=" + id + "&index=" + index
		})
		.done(function( data ) {
...
function getArticoli(){
	...
	$('#articolilist').load("/ContaWeb_Prova_Fabio/articoliFindList.do");
}
...
function setArticolo(){
	...
	$.ajax({
		  ...
		  contentType: "application/json",
		  dataType: "json",
		  url: "/ContaWeb_Prova_Fabio/articolo.do?id=" + id + "&otherId=" + idCliente
		})
		.done(function( data ) {
...
function printDdtMod()
{
	...
	$.ajax({
		  ...
		  contentType: "application/json",
		  dataType: "json",
		  url: "/ContaWeb_Prova_Fabio/printDdt.do?id=" + id
		})
		.done(function( data ) {			
...
function saveDdtNew()
{
	if(checkSaveDdt()) {
		...
		$.ajax({
			  method: "POST",
			  dataType: "json",
			  url: "/ContaWeb_Prova_Fabio/salvaDDT.do",
			  data: serObj
			})
			.done(function( data ) {			
				console.log( data );
			...
			.done(function( data ) {			
				...
				if(data.success === true){				
					...
					$.ajax({
						  ...
						  contentType: "application/json",
						  dataType: "json",
						  url: "/ContaWeb_Prova_Fabio/printDdt.do?id=" + data.id
						})
						.done(function( data ) {			
...
function aggiornaListaDDTs() {
	...
	reportDiv.href = '/ContaWeb_Prova_Fabio/aggiornaDDTs.do?fattura.idCliente=' + idCliente + '&fattura.data=' + dataFattura;
...
dojo.event.topic.subscribe(
	'/calcolaTotaleFattura',function() {
		...
		reportDiv.href="/ContaWeb_Prova_Fabio/calcolaTotaleFattura.do?fattura.sconto=" + scontoFattura;
...
function cancellaGiacenze(){
	...
	if (answer != "0") {
		...
		if(checkedLength != 0){
			...
			var url = "/ContaWeb_Prova_Fabio/giacenzaEdit_input.do?action=deleteBulk";
