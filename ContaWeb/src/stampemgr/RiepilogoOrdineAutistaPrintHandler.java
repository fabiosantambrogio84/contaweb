...
public class RiepilogoOrdineAutistaPrintHandler extends DocumentPrintHandler {

	private Autista autista = null;
	private String date = null;
	private Ordine ordine = null;
	
	public RiepilogoOrdineAutistaPrintHandler(VOElement element, String data) {
		autista = (Autista) element;
		date = data;
	}
	...
	private Map preparaParametriStampa() throws DataAccessException {
		...
		autista = new Autisti().find(autista.getId());
		params.put("autistaNome",autista.getNome());
	...
	private JRDataSource preparaDettagliStampa() throws DataAccessException {
		...
		Iterator itr = new Ordini().getListaOrdiniByAutistaDate(autista.getId(), date).iterator();
	...
	public byte[] creaPDF() throws Exception {
		...
		try {
			InputStream jspTemplate = ServletActionContext.getServletContext().getResourceAsStream("/stampe/RiepilogoOrdineAutista.jasper");
	...
	public String buildPDFPath(boolean mkdirs) {
		...
		String percorsoFile = percorsoMese + ps + "Ordine_riepilogo_autista" + autista.getId() + ".pdf"; 		
		return percorsoFile;
	}
