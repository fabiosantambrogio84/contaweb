...
public class PrintFatture extends PrintPDF {
	...
	public String printNoPec() { //STAMPA LISTA DOCUMENTI
		try {
			Fatture fatture = new Fatture();
			Collection listaFatture = fatture.getFattureNoPEC(dataDal, dataAl);
...
//	}
	...
	public static MimeMessage buildMail(Fattura fattura, String percorsoFattura, Session session) throws AddressException, MessagingException{
		...
		if(fattura.getCliente().getEmailPec() != null && fattura.getCliente().getEmailPec().trim().length() > 0) message.addRecipient(Message.RecipientType.TO,new InternetAddress(fattura.getCliente().getEmailPec()));
		else message.addRecipient(Message.RecipientType.TO,new InternetAddress(fattura.getCliente().getEmail()));
	...
	public String printRiepilogoFatture() throws Exception{
		...
		try {
			...
			if(listaFatture.size() == 0){return ERROR;}
	...
	public String printRiepilogoFattureCC() throws Exception {
		try {
			...
			while (itr.hasNext()) {
				...
				{
					Iva iva = (Iva)obj;
					if(!imponibili.containsKey(iva.getValore())) continue;
	...
	public String printListaFattureMail() {
	    try{
            ...
            logger.info("Prepara invio email. Host="+host+", protocol=smtps, port="+port+", username="+username+", password="+password+", pec=false");
            
            /* prepara l'invio email */
            FattureEmailSender emailSender = new FattureEmailSender(listaFatture, fatture, "smtps", host, port, username, password, true, false);
...
//            newCachedThreadPool.submit(new FattureEmailSenderTask(emailSender));
	...
	public String printListaFatturePec() {
        try{
            ...
            logger.info("Lista fatture ottenuta. Numero elementi: "+listaFatture.size());
            logger.info("Prepara invio email. Host="+host+", protocol=smtps, port="+port+", username="+username+", password="+password+", pec=true");
            
            /* prepara l'invio email PEC */
            FattureEmailSender emailSender = new FattureEmailSender(listaFatture, fatture, "smtps", host, port, username, password, true, true);
