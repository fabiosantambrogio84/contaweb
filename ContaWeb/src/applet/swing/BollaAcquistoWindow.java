...
public class BollaAcquistoWindow {
	...
	public JPanel getPanelDettagli() {
		if (panelDettagli == null) {
			...
			panelDettagli.setBorder(BorderFactory.createTitledBorder(null, "Dettagli", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Tahoma", Font.PLAIN, 11), new Color(0, 70, 213)));
			...
			bNuovoArticolo.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (selectedFornitore == null) {
						JOptionPane.showMessageDialog(getPanelDettagli(), "Selezionare un fornitore","Errore",JOptionPane.ERROR_MESSAGE);
					} else
	...
	private boolean controllaRigaCorretta() {
		...
		try {
			...
			JOptionPane.showMessageDialog(getPanelDettagli(), "Impostare il prezzo dell'articolo","Errore",JOptionPane.ERROR_MESSAGE);
		...
		if (getTxtScontoArticolo().getText().length()>0) {
			try {
				...
				JOptionPane.showMessageDialog(getPanelDettagli(), "Impostare lo sconto dell'articolo","Errore",JOptionPane.ERROR_MESSAGE);
						...
						try {
							if (!txtCodiceArticolo.getText().equalsIgnoreCase("")) {
								...
								if(getCmbModalita().getSelectedIndex() == 0)
									...
									selectedArticolo = new DbConnector().getArticoloByBarCode(txtCodiceArticolo.getText().substring(0, 7),selectedFornitore);
								...
								if (selectedArticolo != null) {
									...
									else 
									{
										...
										if (!selectedArticolo.isCompleteBarCode()) { //l'articolo non ha una qta predefinita
											...
											double value = (long) dfUS.parse(barcodeStr.substring(barcodeStr.length() - 6, barcodeStr.length()));
											value = value / 10000;
			...
			bNuovaBolla.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (table.getRowCount() == 0 || JOptionPane.showConfirmDialog(bNuovaBolla, 
							"Attenzione! Le eventuali modifiche correnti verranno annullate. " +
							"Si desidera continuare?", "Attenzione!",
			...
			bSalvaBolla.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						JOptionPane.showMessageDialog(getPanelBottoni(), "La bolla � stata salvata correttamente","Informazione",JOptionPane.INFORMATION_MESSAGE);
	...
	private BollaAcquisto getBollaAcquisto() throws Exception {
		...
		try {
			...
			for (int i=0; i < getTable().getRowCount(); ++i) {
				...
				dba.setQta(new BigDecimal(nf.parse((String) getTable().getValueAt(i, 2)).doubleValue()).setScale(3, BigDecimal.ROUND_HALF_UP));
				dba.setPrezzo(new BigDecimal(nf.parse((String) getTable().getValueAt(i, 3)).doubleValue()).setScale(2, BigDecimal.ROUND_HALF_UP));
				if (getTable().getValueAt(i, 4) != null) {
					dba.setSconto(new BigDecimal(nf.parse((String) getTable().getValueAt(i, 4)).doubleValue()).setScale(2, BigDecimal.ROUND_HALF_UP));
				}
