...
public class DDTWindow {
		...
		for(int i=0;i<table.getRowCount();++i) {
			...
			if (table.getModel().getValueAt(i, COL_ID_ORDINI) != null) {
				if (table.getModel().getValueAt(i, COL_ID_ORDINI) instanceof Collection)
					dettaglioDDT.getEvasioniOrdini().addAll((Collection<? extends EvasioneOrdine>) table.getModel().getValueAt(i, COL_ID_ORDINI));
				else
					dettaglioDDT.getEvasioniOrdini().add((EvasioneOrdine) table.getModel().getValueAt(i, COL_ID_ORDINI));
			}
	...
	private JPanel getPanelSpedizione() {
		if (panelSpedizione == null) {
			...
			panelSpedizione.setBorder(BorderFactory.createTitledBorder(null, "Spedizione", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Tahoma", Font.PLAIN, 11), new Color(0, 70, 213)));
			...
			/*c.gridy = 2;
			panelSpedizione.add(labelAspettoEst, c);*/
			c.gridx = 1;
			c.gridy = 0;
			c.anchor = GridBagConstraints.WEST;
			...
			/*c.gridy = 2;
			panelSpedizione.add(getTxtAspettoEsteriore(), c);*/
			c.gridx = 2;
			c.gridy = 0;
			c.weightx = 1;
			c.anchor = GridBagConstraints.EAST;
			...
			/*c.gridy = 2;
			panelSpedizione.add(labelCausale, c);*/
			c.gridx = 3;
			c.gridy = 0;
			c.weightx = 0;
			c.anchor = GridBagConstraints.WEST;
			...
			/*c.gridy = 2;
			c.fill = GridBagConstraints.HORIZONTAL;
			panelSpedizione.add(getTxtCausale(), c);*/
		}
	...
	/*private JTextField getTxtAspettoEsteriore() {
		if (txtAspettoEsteriore == null) {
			txtAspettoEsteriore = new JTextField();
			txtAspettoEsteriore.setText("scatole");
			txtAspettoEsteriore.setColumns(6);
			txtAspettoEsteriore.setMinimumSize(new Dimension(70,21));
		}
		return txtAspettoEsteriore;
	}*/
	...
	/*private JTextField getTxtCausale() {
		if (txtCausale == null) {
			txtCausale = new JTextField();
			txtCausale.setText("vendita");
			txtCausale.setColumns(6);
			txtCausale.setMinimumSize(new Dimension(90,21));
		}
		return txtCausale;
	}*/
			...
			buttonModifica.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						int update = JOptionPane.showConfirmDialog(contentPane, "Vuoi aggiornare l'ordine?", "Conferma", JOptionPane.YES_NO_OPTION);
						new DbConnector().saveDDT(ddt, update == 0);
						JOptionPane.showMessageDialog(contentPane, "Il DDT � stato aggiornato con successo","Successo",JOptionPane.INFORMATION_MESSAGE);
						buttonModifica.setEnabled(false);
					} catch (Exception e1) {
			...
			buttonSN.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						JOptionPane.showMessageDialog(contentPane, "Il DDT � stato salvato con successo","Successo",JOptionPane.INFORMATION_MESSAGE);
						setModalitaInserimentoDDT();
					} catch (Exception e1) {
						JOptionPane.showMessageDialog(contentPane, e1.getMessage(),"Attenzione",JOptionPane.ERROR_MESSAGE);
					}
			...
			table.addMouseListener(new MouseAdapter() {
							...
							if (model.getValueAt(selectedRow, COL_ID_ORDINI) != null) {
								try {
									if (model.getValueAt(selectedRow, COL_ID_ORDINI) instanceof ArrayList) {
										...
										int res = JOptionPane.showConfirmDialog(table,message,"Attenzione!",JOptionPane.OK_CANCEL_OPTION);
										...
										String message = "Si desidera cancellare questo articolo dall'ordine " + eo.getDettaglioOrdine().getIdOrdine() + "?";
										int res = JOptionPane.showConfirmDialog(table,message,"Attenzione!",JOptionPane.OK_CANCEL_OPTION);
				...
				private void cancellaRigaOrdine(EvasioneOrdine eo) {
					if (new DbConnector().cancellaRigaOrdine(eo)) {
						JOptionPane.showMessageDialog(table, "Cancellazione avvenuta con successo", "Informazione", JOptionPane.INFORMATION_MESSAGE);
						//Aggiorna riga sulla tabella
						DettagliTableModel model = (DettagliTableModel) table.getModel();
						int selectedRow = table.getSelectedRow();
						
						if ((model.getValueAt(selectedRow, COL_LOTTO) != null && ((String)model.getValueAt(selectedRow, COL_LOTTO)).equalsIgnoreCase(""))
	...
	public void loadOrdineCliente() {
		...
		dettagliOrdini = new DbConnector().getListaDettagliOrdini(clienteSelezionato.getId(), idPuntoConsegna, getTxtDataTrasporto().getText());
		Iterator itr = dettagliOrdini.iterator();
		Set noteMostrate = new HashSet();
		while (itr.hasNext()) {
			DettaglioOrdine det = (DettaglioOrdine) itr.next();
			
			if (det.getOrdine() != null 
					&& !noteMostrate.contains(det.getIdOrdine().intValue())
					&& det.getOrdine().getNote() != null
					&& !det.getOrdine().getNote().equalsIgnoreCase("")) {
				JOptionPane.showMessageDialog(contentPane, "Nota ordine " + det.getOrdine().getProgressivoCompleto() +":" + "\n" + det.getOrdine().getNote());
			...
			if (values.containsKey(det.getIdArticolo())) {
				...
				PrezzoConSconto prezzoArticolo = new DbConnector().getPrezzoArticolo(det.getArticolo().getCodiceArticolo(), clienteSelezionato.getId());
				NumberFormat formatter = DecimalFormat.getInstance(Locale.ITALIAN);
				formatter.setMinimumFractionDigits(2);
