...
public class Clienti extends DataAccessObject {
	...
	public void store(Cliente value) throws DataAccessException {
		...
		if (value.getId() != null && value.getTelefonate() != null) {
			Cliente cliente = new Cliente();
			cliente.setId(value.getId());
			cliente = (Cliente) findWithAllReferences(cliente);
			
			if (cliente.getTelefonate() != null) {
				Iterator itr = cliente.getTelefonate().iterator();
				while (itr.hasNext()) {
					Telefonata tel = (Telefonata) itr.next();
					Iterator itr2 = value.getTelefonate().iterator();
					boolean trovato = false;
					while (itr2.hasNext() && !trovato) {
						Telefonata tel2 = (Telefonata) itr2.next();
						if (tel2.getGiorno().intValue() == tel.getGiorno().intValue()) {
							trovato = true;
							tel2.setIdCliente(tel.getIdCliente());
							tel2.setCliente(tel.getCliente());
