...
public class Ordini extends DataAccessObject {
	...
	public String store(VOElement element) throws DataAccessException {
		...
		if (ordine.getId() != null) {
			...
			try {
				...
				while (itr.hasNext()) {
					...
					if (!dettagliAttivi.containsKey(det.getIdArticolo().intValue())) { //CANCELLO ID VECCHI
						...
						if(detAttivo.getPezziOrdinati() > detAttivo.getPezziDaEvadere())  nuovo = false;							
						// se una riga non è evasa completamente non è completo..
						if(detAttivo.getPezziDaEvadere() > 0) completo = false; 
					}
				}
				
				if(nuovo) ordine.setStatoOrdine(Ordine.DA_EVADERE);
				else if(completo) ordine.setStatoOrdine(Ordine.EVASO);
				else ordine.setStatoOrdine(Ordine.PARZIALMENTE_EVASO);
			...
			try {
				...
				ReportQueryByCriteria q = QueryFactory.newReportQuery(Ordine.class, new String[] { "max(numeroProgressivo)"}, crit,true);
    ...
    public Collection<DettaglioOrdine> getDettagliPerCliente(Integer idCliente, Integer idPuntoConsegna, Date dataConsegna) throws DataAccessException {
    ...
    public Collection getDettagliAncoraDaEvadere(DettaglioDDT dt) throws DataAccessException {
		...
		Collection ordiniNuovi = getDettagliPerCliente(dt.getDdt().getIdCliente(), dt.getDdt().getIdPuntoConsegna(), dt.getDdt().getDataTrasporto());
	...
	public void aggiornaEvasioni(int pezziDaAggiungere, DettaglioDDT dt, PersistenceBroker broker) throws DataAccessException {
			...
			while (itr2.hasNext() && pezziDaAggiungere != 0) {
				...
				if(ordine != null) store(ordine);
				broker.store((DettaglioOrdine)det);
				broker.store((EvasioneOrdine)eo);
				broker.commitTransaction();
			}
	...
	/* ***
		public Collection setOrderByDaEvadere(int order) throws DataAccessException {
			if (order == ORDER_DESC)
			{
				getQueryByCriteria().addOrderByDescending("dataCreazione");
			}
			else
			{
				getQueryByCriteria().addOrderByAscending("dataCreazione");
	        ...
	        while(itr.hasNext()) 
	        {
	        	...
	        	if(element.getStato() == 1)
	        	{
	        		warning_obj.add(element);
	        	}
	        	else
	        	{
	        		normal_obj.add(element);
	        	}
