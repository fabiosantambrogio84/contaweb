...
public class EditStatistiche extends List {
	...
	private static final Logger logger = Logger.getLogger(EditStatistiche.class);
	...
	private java.util.List<Integer> idClienti = null;
...
//	}
	...
	public String getClientiRs(){
		return clientiRs;
	}
	
	public void setClientiRs(String clientiRs){
		this.clientiRs = clientiRs;
	}
...
//	   }
	   ...
	   if(idClienti != null && !idClienti.isEmpty()){
		   vendite.setClienti(idClienti);
	   }
...
//	}
	...
	public java.util.List<Integer> getIdClienti() {
		return idClienti;
	}
	
	public void setIdClienti(java.util.List<Integer> idClienti) {
		this.idClienti = idClienti;
		if(idClienti != null && !idClienti.isEmpty()){
			String clientiRs = "";
			for(Integer id : idClienti){
				try{
					Cliente cliente = new Clienti().find(id);
					clientiRs = clientiRs + cliente.getRs() + ","; 
				} catch(Exception e){
					logger.error("Errore nel recupero del cliente con id '"+id+"'");
				}
			}
			if(clientiRs != null && clientiRs != ""){
				if(clientiRs.substring(clientiRs.length()-1).equals(",")){
					clientiRs = clientiRs.substring(0, clientiRs.length()-1);
				}
				setClientiRs(clientiRs);
			}
		}
	}
}