   ...
   private Collection listaDDTs = null;
	...
	public String calcolaTotaleFattura() throws DataAccessException {
		...
		if (fattura != null && fattura.getSconto() != null && fattura.getSconto().compareTo(new BigDecimal(0)) != 0) {
			BigDecimal sconto;
			sconto = totaleFattura.multiply(fattura.getSconto().divide(new BigDecimal(100).setScale(2, BigDecimal.ROUND_HALF_UP)).setScale(2,BigDecimal.ROUND_HALF_UP));
	 	...
	 	try {
	 		...
	 		if (getAction().equalsIgnoreCase("insert")) {
		 		...
		 		while (itr.hasNext()) {
		 			...
		 			if(ddt.getPagato() == null || !ddt.getPagato()) pagata = false;
	 		...
	 		addActionError("Errore: numero di fattura già esistente!");
			return ERROR;
		}
	 	catch (Exception e) {
