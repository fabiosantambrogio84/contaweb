...
public class Ordine extends VOElement {
    ...
    static public final int EVASO = 10;
    ...
    public Integer getStato() {
        /*
         * *** if (getDettagliOrdine() != null) for(int i=0; i< dettagliOrdine.size(); ++i) if
         * (((DettaglioOrdine)dettagliOrdine.get(i)).getPezziDaEvadere()>0) return WARNING;
         */

        if (getStatoOrdine() == DA_EVADERE)
            return WARNING;
        else if (getStatoOrdine() == PARZIALMENTE_EVASO)
            return 4;
        else if (getStatoOrdine() == EVASO)
            return NORMAL;
