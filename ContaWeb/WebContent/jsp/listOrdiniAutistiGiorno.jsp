...
<tiles:importAttribute name="list-autisti" scope="request" ignore="true"/>
...
<label id="messageBox" style="margin-left: 20px"></label>
	...
	<%
				DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy", Locale.ITALIAN);
						...
						Iterator itr = ((Collection)request.getAttribute("list-autisti")).iterator();
						while (itr.hasNext()) {
					
					String dataprecedente = "";
					Autista autista = (Autista)itr.next();
						
					Iterator itr_ord = new Ordini().getListaOrdiniperautistadata(autista.getId()).iterator();
					
					if(new Ordini().getListaOrdiniperautistadata(autista.getId()).size() > 0)
					{
			%>
				<div class="veryBig24 bgSilver bold upper"><%=autista.getNome() %></div>
				...
				while(itr_ord.hasNext())
				{
					...
					//autista.setData(ord.getDataSpedizione().toString());
					...
					if(!dataprecedente.equals(dataadesso))
					{
						...
						<tr class="tableCell veryBig bgBlack white">
							...
							<a style="float:right" href="printRiepilogoOrdineAutista.do?id=<%=autista.getId()%>&data=<%=ord.getDataSpedizione() %>" class="link">Stampa giornata</a>
							</td>
						</tr>
						<tr>
					...
					<tr class="normalRow" id="tr_<%=ord.getId()%>">
						...
						<a href="printOrdineAutista.do?id=<%=ord.getId()%>" class="link">Stampa</a>
						</td>
